...
@push('styles')
    <style>
        ...
        .timeline-item p { word-break: break-word; white-space: normal; }
        ...
        .timeline-dot.active  { background: #3b82f6; box-shadow: 0 0 0 2px #bfdbfe; color: white; }
        .timeline-dot.pending { background: #f1f5f9; box-shadow: 0 0 0 2px #e2e8f0; color: #94a3b8; }
        .timeline-dot.cancelled { background: #ef4444; box-shadow: 0 0 0 2px #fecaca; color: white; }
    </style>
@endpush
...
<div class="max-w-3xl mx-auto space-y-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div class="space-y-4">
            ...
            <div class="card p-5">
                ...
                <div>
                    ...
                    <div class="info-row">
                        ...
                        <span class="info-value">{{ $order->vehicle_type === 'motor' ? '🏍️ Motor' : '🚗 Mobil' }}</span>
                    </div>
                    <div class="info-row">
                        ...
                        <span class="info-value capitalize">
                            ...
                            @endphp
                            <span class="badge badge-{{ $isPaid ? 'success' : ($isCash ? 'info' : 'warning') }} ml-1" style="font-size:10px">
                                @if ($isCash)
                                    @if ($order->status === 'completed') ✅ Lunas (Cash)
                    ...
                    @if ($order->completed_at)
                        <div class="info-row">
                            ...
                            <span class="info-value text-sm">{{ \Carbon\Carbon::parse($order->completed_at)->format('d M Y, H:i') }}</span>
                        </div>
                    @endif
                    @if ($order->cancelled_at)
                        <div class="info-row">
                            <span class="info-label">Dibatalkan</span>
                            <span class="info-value text-sm text-red-500">{{ \Carbon\Carbon::parse($order->cancelled_at)->format('d M Y, H:i') }}</span>
                        </div>
                    @endif
            ...
            <div class="card p-5">
                ...
                <div class="space-y-3">
                    <div class="flex items-start gap-3">
                        <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5" style="background:#dcfce7">
                        ...
                        <div>
                            ...
                            @if ($order->pickup_lat && $order->pickup_lng)
                                <p class="text-xs text-slate-400 mt-0.5">{{ $order->pickup_lat }}, {{ $order->pickup_lng }}</p>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                        <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5" style="background:#fee2e2">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-semibold text-slate-700 mt-0.5">{{ $order->destination_address }}</p>
                            @if ($order->destination_lat && $order->destination_lng)
                                <p class="text-xs text-slate-400 mt-0.5">{{ $order->destination_lat }}, {{ $order->destination_lng }}</p>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <div class="card p-5">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="font-semibold">Rp {{ number_format($order->service_fee ?? 0, 0, ',', '.') }}</span>
                    </div>
                    <div class="border-t border-slate-100 pt-2 flex justify-between font-black">
                        <span>Total</span>
                        <span style="color:#16a34a">Rp {{ number_format($order->total_fare ?? 0, 0, ',', '.') }}</span>
                    </div>
                    <div class="border-t border-dashed border-slate-100 pt-2 space-y-1.5">
                        <div class="flex justify-between text-xs">
                            <span class="text-slate-400">Platform Fee (20%)</span>
                            <span class="font-bold text-slate-600">Rp {{ number_format($order->platform_fee ?? 0, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between text-xs">
                            <span class="text-slate-400">Pendapatan Mitra (80%)</span>
                            <span class="font-bold" style="color:#16a34a">Rp {{ number_format($order->mitra_earning ?? 0, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {{-- PEMBAYARAN --}}
            @if (in_array($order->payment_method, ['qris', 'transfer', 'e_wallet']))
                @php
                    $ewalletData = [
                        'gopay'     => ['name' => 'GoPay',     'color' => '#00AED6'],
                        'ovo'       => ['name' => 'OVO',        'color' => '#4c3494'],
                        'dana'      => ['name' => 'DANA',       'color' => '#1185e0'],
                        'shopeepay' => ['name' => 'ShopeePay',  'color' => '#ef4444'],
                        'linkaja'   => ['name' => 'LinkAja',    'color' => '#e8282b'],
                        'astrapay'  => ['name' => 'AstraPay',   'color' => '#0d4f9e'],
                    ];
                    $ewallet = $ewalletData[$order->ewallet_selected ?? ''] ?? null;
                    $paymentTitle = match ($order->payment_method) {
                        'qris'     => '📱 Pembayaran QRIS',
                        'transfer' => '🏦 Pembayaran Transfer Bank',
                        'e_wallet' => '💳 Pembayaran ' . ($ewallet['name'] ?? 'E-Wallet'),
                        default    => '💰 Pembayaran',
                    };
                @endphp
                <div class="card p-5">
                    ...
                    @if ($order->isPaid())
                        <div class="flex items-center gap-3 p-3 rounded-xl" style="background:#f0fdf4;border:1px solid #bbf7d0">
                            ...
                            <div>
                                ...
                                <p class="text-xs" style="color:#16a34a">Dikonfirmasi {{ $order->paid_at?->format('d M Y, H:i') }}</p>
                            </div>
                        </div>
                    ...
                    @elseif($order->payment_proof)
                        <div class="space-y-3">
                            <p class="text-xs font-bold text-slate-500 uppercase tracking-wide">
                                ...
                                @if ($order->payment_method === 'e_wallet' && $ewallet)
                                    <span style="color:{{ $ewallet['color'] }}">({{ $ewallet['name'] }})</span>
                                @endif
                            </p>
                            <img src="{{ $order->payment_proof_url }}"
                                class="w-full max-w-xs rounded-xl border border-slate-200 cursor-pointer"
                    ...
                    @else
                        <div class="p-3 rounded-xl text-sm font-semibold"
                            ...
                            @if ($order->payment_method === 'e_wallet' && $ewallet) via {{ $ewallet['name'] }}
        ...
        <div class="space-y-4">
                    ...
                    @if ($order->customer_rating)
                        <div class="mt-3 pt-3 border-t border-slate-100">
                            ...
                            <div class="flex items-center gap-1">
                                @for ($i = 1; $i <= 5; $i++)
                                    <i class="fas fa-star text-sm {{ $i <= $order->customer_rating ? 'text-amber-400' : 'text-slate-200' }}"></i>
                                @endfor
                                <span class="text-xs font-bold text-slate-600 ml-1">{{ $order->customer_rating }}/5</span>
                            </div>
                    ...
                    @if ($order->mitra->mitraProfile)
                        <div class="mt-3 pt-3 border-t border-slate-100 grid grid-cols-2 gap-2 text-xs">
                            <div>
                                ...
                                <p class="font-bold text-slate-700 mt-0.5">{{ $order->mitra->mitraProfile->vehicle_brand ?? '–' }}</p>
                            </div>
                            <div>
                                <span class="text-slate-400">Plat Nomor</span>
                                <p class="font-bold text-slate-700 mt-0.5">{{ $order->mitra->mitraProfile->vehicle_plate ?? '–' }}</p>
                            </div>
                            <div>
                                <span class="text-slate-400">Rating</span>
                                <p class="font-bold mt-0.5" style="color:#f59e0b">⭐ {{ $order->mitra->mitraProfile->rating ?? '5.0' }}</p>
                            </div>
                            <div>
                                <span class="text-slate-400">Total Trip</span>
                                <p class="font-bold text-slate-700 mt-0.5">{{ $order->mitra->mitraProfile->total_trips ?? 0 }}x</p>
                            </div>
                        </div>
                    @endif
                @else
                    ...
                    <div class="space-y-2">
                        @foreach ($order->items as $item)
                            <div class="flex justify-between items-center py-1.5 border-b border-slate-50 last:border-0">
                                ...
                                <p class="font-bold text-sm text-slate-800">Rp {{ number_format($item->subtotal, 0, ',', '.') }}</p>
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            @if (!in_array($order->status, ['completed', 'cancelled']))
                <div class="card p-5">
                    ...
                    <p class="text-xs text-slate-400 mb-3">Admin dapat mengubah status pesanan secara paksa jika diperlukan.</p>
                    ...
                    <div class="timeline-item">
                        ...
                        <div>
                            ...
                            @if ($tracking)
                                ...
                                <p class="text-xs text-slate-400 mt-0.5">{{ $tracking->created_at->format('d M Y, H:i') }}</p>
                            @elseif(!$isDone)
